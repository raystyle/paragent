...
name: par
license: MIT
description: "herdr 并行 agent 编排（薄路由；实现=全局 CLI `par`）：五原语 dev 并行分支/research 研究/review 审核/ix 交流/triad 三席。TRIGGER when: 并行分支开叉/合作交流/研究分析/review 审核、wave/run/ix/triad、par_result、并行 merge/discard。BLOCKING: 一切并行编排除查状态外一律 `par` CLI（references/parallel.md 为正本）。SKIP: 工作台修台/布局/mirror（workspace skill）、凭据/Vault（center）。"
...
7. 交互席（ix/triad）仅 `ix close` / `triad close` 关；`close-tasks` 跳过它们。

## 路由

| 要做什么 | 命令 |
|---|---|
| 并行分支/开叉 dev | `par wave --mode dev t1=@dev/a t2=@dev/b` → `par verify <tid> --cmd …` → 人确认 → `par merge` |
...
| 单任务 | `par run <tid> @dev/a [--mode …] [--brief "…"]` |
| 并行交流 ix | `par ix open` → `par ix fire a\|b "…"` → `par ix take\|collect` → `par ix close` |
| 三席 triad | `par triad open [--mode …]` → `par triad fire "题"` → `take\|collect` → `par triad close` |
| 归档回看 | `par ix archive [--json]` |
...
`references/parallel.md`（正本原语卡）· `DISCIPLINE.md` · `{dev,research,review,interactive,triad}-mode.md` · `models.md` · `production-roadmap.md`
