...
| 原语 | 何时 | 一条命令 |
|---|---|---|
| **dev** 并行分支/开叉 | 多任务零文件重叠改代码 | `par wave --mode dev t1=@dev/a t2=@dev/b` → verify → 人确认 → merge |
...
| **ix** 并行合作/交流 | 主窗人控 + 右双席异步 | `par ix open` → `ix fire a\|b` → `ix take\|collect` → `ix close` |
...
par smoke [--live]  # 冒烟（stub 全量 + 可选 live ix token 轮）
...
- 协议细则：`references/{dev,research,review,interactive,triad}-mode.md` · `DISCIPLINE.md`
