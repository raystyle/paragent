#!/usr/bin/env bash
# par-discuss.sh — 并行交互（两令制 · 异步递进）
# 用法:
#   par-discuss.sh open [--a <轨|cmd>] [--b <轨|cmd>] [--cwd DIR] [--force]
#   ── 两令（多任务并行唯一节奏）──
#   par-discuss.sh fire <a|b> <msg…>     # 令1：只派发，火即返（=prompt，禁默认齐等）
#   par-discuss.sh take [--all] [--read] # 令2：只收割已 READY（非阻塞；无则 rc3）
#   ── 编排糖（非第三令；= wait + take，不污染 fire）──
#   par-discuss.sh collect [--all|a|b] [--read] [--timeout-ms N]
#   ── 辅助 ──
#   par-discuss.sh status|poll [--json]
#   par-discuss.sh wait [--any|a|b|all]  # 可选阻塞；讨论期优先 take 勿 wait all
#   par-discuss.sh archive [--json]      # 归档清单/汇总（rc3=无归档）
#   par-discuss.sh close
#
# 默认美学: 右上 a=@review/a Opus4.8 · 右下 b=@review/b GPT顶档
# 主窗不代开。交互期不关右窗，仅 discuss close。
set -uo pipefail
...
DISCUSS_ROOT="${PAR_DISCUSS_DIR:-$PWD/.parallel/discuss}"
DISCUSS_CLAUDE_DIR="${DISCUSS_ROOT%/discuss}/discuss-claude"
DISCUSS_CODEX_DIR="${DISCUSS_ROOT%/discuss}/discuss-codex"
LABEL_CLAUDE="discuss-claude"
LABEL_CODEX="discuss-codex"
...
_discuss_slot() {
  case "$(printf '%s' "${1:-}" | tr 'A-Z' 'a-z')" in
    claude|a|left|opus|review/a|discuss-claude) echo claude ;;
    codex|b|right|gpt|review/b|discuss-codex)   echo codex ;;
    all|both) echo all ;;
    any|first|"") echo any ;;
...
_discuss_round_file() {
  local d; d=$(_discuss_dir "$1") || return 1
  printf '%s/round' "$d"
}

_discuss_round_tid() {
  # 席级 tid：discuss-claude / discuss-codex（与 @review 轨解耦，仅作 token 锚）
  printf 'discuss-%s' "$1"
}

_discuss_round_write() {
  local role=$1 pending=${2:-0} busy=${3:-0} harv=${4:-0}
  local tid=${5:-} att=${6:-} f cur_t cur_a
  f=$(_discuss_round_file "$role") || return 1
  mkdir -p "$(dirname "$f")"
  if [ -z "$tid" ] || [ -z "$att" ]; then
    cur_t=$(sed -n 's/^tid=//p' "$f" 2>/dev/null | head -1)
    cur_a=$(sed -n 's/^attempt=//p' "$f" 2>/dev/null | head -1)
    tid=${tid:-${cur_t:-$(_discuss_round_tid "$role")}}
    att=${att:-${cur_a:-0}}
  fi
  printf 'pending=%s\nbusy_seen=%s\nharvestable=%s\ntid=%s\nattempt=%s\n' \
...
_discuss_round_read() {
  ...
  f=$(_discuss_round_file "$1") || { echo "0 0 0  0"; return; }
  if [ -f "$f" ]; then
...
_discuss_next_attempt() {
  local att
  att=$(sed -n 's/^attempt=//p' "$(_discuss_round_file "$1")" 2>/dev/null | head -1)
  echo $(( ${att:-0} + 1 ))
}

# fire：pending + 新 attempt；清旧 par_result 防串台
_discuss_round_mark_prompt() {
  local role=$1 pane=$2 tid att
  tid=$(_discuss_round_tid "$role")
  att=$(_discuss_next_attempt "$role")
  [ -n "$pane" ] && par_clear_result_token "$pane"
  _discuss_round_write "$role" 1 0 0 "$tid" "$att"
  printf '%s %s\n' "$tid" "$att"
}

_discuss_round_clear() {
  _discuss_round_write "$1" 0 0 0
}

# token 判本轮是否可收：done|blocked|waiting
_discuss_token_gate() {
  local role=$1 pane=$2 pending busy harv tid att r
  read -r pending busy harv tid att < <(_discuss_round_read "$role")
...
_discuss_try_harvest_from_token() {
  local role=$1 pane=$2 gate
  gate=$(_discuss_token_gate "$role" "$pane")
  case "$gate" in
    done|blocked)
      _discuss_round_write "$role" 0 0 1
      return 0 ;;
  esac
  return 1
}
...
_discuss_round_mark_harvestable() {
  _discuss_round_write "$1" 0 0 1
}

# status/poll 展示用：未 fire 的 idle 可「先看」；已 fire 则须 token 才 READY
# take 一律走 _discuss_round_takeable（更严）
_discuss_round_ready() {
  local role=$1 st=$2 pane=$3 pending busy harv tid att
  read -r pending busy harv tid att < <(_discuss_round_read "$role")
  if [ "$pending" = 1 ]; then
    case "$st" in
      working)
        _discuss_round_write "$role" 1 1 0
        return 1 ;;
      done|blocked|idle)
        [ "$st" = idle ] && [ "$busy" != 1 ] && return 1
        [ "$st" = idle ] && _discuss_round_write "$role" 1 1 0
        if [ -n "$pane" ] && _discuss_try_harvest_from_token "$role" "$pane"; then
          return 0
        fi
        return 1 ;;
      *) return 1 ;;
  ...
  if [ "$harv" = 1 ]; then
    [ -n "$pane" ] || return 0
    case "$(_discuss_token_gate "$role" "$pane")" in done|blocked) return 0 ;; *) return 1 ;; esac
  fi
  # 本轮未 fire（attempt=0）：idle 可先看，不可 take
  _discuss_is_ready "$st"
  return $?
}

# take 专用：仅 fire 本轮且 par_result 锚定 tid#attempt 非空
_discuss_round_takeable() {
  local role=$1 st=$2 pane=$3 pending busy harv tid att
  # 推进 busy_seen；token 过闸才 harvestable
  _discuss_round_ready "$role" "$st" "$pane" >/dev/null 2>&1 || true
  read -r pending busy harv tid att < <(_discuss_round_read "$role")
  if [ "$harv" = 1 ]; then
    # 再验 token（防 take 前被清）
    case "$(_discuss_token_gate "$role" "$pane")" in
      done|blocked) return 0 ;;
      *) _discuss_round_write "$role" 0 0 0; return 1 ;;
    esac
  fi
  return 1
}
...
_discuss_round_took() {
  local role=$1 pane=$2
  _discuss_round_write "$role" 0 0 0
  [ -n "$pane" ] && par_clear_result_token "$pane"
}

# agent wait 返回后：只认 token，不单靠 agent_status
_discuss_round_complete() {
  local role=$1 pane=$2
  _discuss_round_write "$role" 1 1 0
  _discuss_try_harvest_from_token "$role" "$pane" || true
}

# prompt 后短等 busy，尽量记下 busy_seen（不堵死异步；不据此 harvest）
_discuss_arm_after_prompt() {
  local role=$1 pane=$2 st i max="${PAR_DISCUSS_ARM_MS:-2000}" step=100 n
  n=$((max / step)); [ "$n" -lt 1 ] && n=1
  for i in $(seq 1 "$n"); do
    st=$(_discuss_agent_status "$pane")
    case "$st" in
...
_discuss_dir() {
  case "$1" in
    claude) echo "$DISCUSS_CLAUDE_DIR" ;;
    codex)  echo "$DISCUSS_CODEX_DIR" ;;
    *) return 1 ;;
  esac
}
...
_discuss_write_side() {
  ...
  d=$(_discuss_dir "$role") || return 1
  mkdir -p "$d"
  printf '%s\n' "$pane" > "$d/pane"
  printf 'mode=discuss\nlayout=stack\nrole=%s\ncmd=%s\nlabel=%s\ncwd=%s\n' \
    "$role" "$cmd" "$(_discuss_label "$role")" "${CWD:-$PWD}" > "$d/meta"
  echo working > "$d/state"
}

_discuss_write_session() {
  ...
  } > "$DISCUSS_ROOT/session"
}

_discuss_load_session() {
  CLAUDE_PANE=$(_discuss_read_pane claude)
  CODEX_PANE=$(_discuss_read_pane codex)
  if [ -f "$DISCUSS_ROOT/session" ]; then
    CLAUDE_CMD=$(sed -n 's/^claude_cmd=//p' "$DISCUSS_ROOT/session" | head -1)
    CODEX_CMD=$(sed -n 's/^codex_cmd=//p' "$DISCUSS_ROOT/session" | head -1)
    CWD=$(sed -n 's/^cwd=//p' "$DISCUSS_ROOT/session" | head -1)
  fi
...
_discuss_print_usage() {
  cat <<EOF
parallel: discuss ready（两令制 · 右 stack · 不自动关窗）
  主窗    （人类自定，不代开）
  右上 a  pane=${CLAUDE_PANE}  cmd=${CLAUDE_CMD}
  右下 b  pane=${CODEX_PANE}   cmd=${CODEX_CMD}
  session $DISCUSS_ROOT/session
...
cmd_open() {
  ...
  local old_c old_x
  old_c=$(_discuss_read_pane claude)
  old_x=$(_discuss_read_pane codex)
  if [ "$force" != 1 ] && _discuss_pane_alive "$old_c" && _discuss_pane_alive "$old_x"; then
    CLAUDE_PANE=$old_c
    CODEX_PANE=$old_x
    _discuss_write_side claude "$CLAUDE_PANE" "$CLAUDE_CMD"
    _discuss_write_side codex  "$CODEX_PANE"  "$CODEX_CMD"
    _discuss_round_clear claude
    _discuss_round_clear codex
    _discuss_write_session
    echo "parallel: discuss reuse claude=$CLAUDE_PANE codex=$CODEX_PANE"
    _discuss_print_usage
    return 0
  fi
  ...
  CLAUDE_PANE=$(par_recruit "$WS" "$LABEL_CLAUDE" "$CLAUDE_CMD" "$CWD" stack) \
    || { err "discuss claude recruit 失败"; exit 2; }
  _discuss_write_side claude "$CLAUDE_PANE" "$CLAUDE_CMD"
  echo "parallel: discuss opened claude pane=$CLAUDE_PANE cmd=$CLAUDE_CMD"

  CODEX_PANE=$(par_recruit "$WS" "$LABEL_CODEX" "$CODEX_CMD" "$CWD" stack) \
    || { err "discuss codex recruit 失败（claude=$CLAUDE_PANE 仍保留）"; exit 2; }
  _discuss_write_side codex "$CODEX_PANE" "$CODEX_CMD"
  echo "parallel: discuss opened codex pane=$CODEX_PANE cmd=$CODEX_CMD"
...
cmd_status() {
  ...
  st_c=$(_discuss_agent_status "${CLAUDE_PANE:-}")
  st_x=$(_discuss_agent_status "${CODEX_PANE:-}")
  # status READY：未 fire 的 idle 可先看；已 fire 须 par_result 才 READY
  _discuss_round_ready claude "$st_c" "${CLAUDE_PANE:-}" && ra=true
  _discuss_round_ready codex "$st_x" "${CODEX_PANE:-}" && rb=true
  if [ "$json" = 1 ]; then
    jq -n \
      --arg mode discuss --arg layout stack --arg pace async \
  ...
  echo "parallel: discuss status (async progressive；take 以 par_result 为准；urgency 序)"
  {
...
cmd_poll() {
  ...
  _discuss_load_session
  [ -n "${CLAUDE_PANE:-}" ] || [ -n "${CODEX_PANE:-}" ] || { err "无 discuss session（先 open）"; exit 2; }
  local st_c st_x
  st_c=$(_discuss_agent_status "${CLAUDE_PANE:-}")
  st_x=$(_discuss_agent_status "${CODEX_PANE:-}")
  # poll：未 fire 的 idle 算可先看；take 仍要 token
  if _discuss_round_ready claude "$st_c" "${CLAUDE_PANE:-}"; then ra=true; ready_n=$((ready_n + 1)); fi
  if _discuss_round_ready codex "$st_x" "${CODEX_PANE:-}"; then rb=true; ready_n=$((ready_n + 1)); fi
  if [ "$json" = 1 ]; then
    jq -n \
      --arg cp "${CLAUDE_PANE:-}" --arg xp "${CODEX_PANE:-}" \
      --arg cs "$st_c" --arg xs "$st_x" \
    ...
    echo "parallel: discuss poll ready_count=$ready_n"
    if [ "$ra" = true ]; then
...
cmd_prompt() {
  local slot msg wait=0 tmo="${PAR_DISCUSS_PROMPT_TIMEOUT_MS:-120000}" role pane
  [ $# -ge 2 ] || { err "usage: par-discuss.sh prompt <a|b|claude|codex> <msg…> [--wait] [--timeout-ms N]"; exit 1; }
  slot=$1; shift
  msg=""
  while [ $# -gt 0 ]; do
    case "$1" in
  ...
  role=$(_discuss_slot "$slot") || exit 1
  case "$role" in
    claude|codex) ;;
  ...
  if [ "$wait" = 1 ]; then
    ...
    _discuss_arm_after_prompt "$role" "$pane"
  fi
  echo "parallel: discuss fire $role pane=$pane tid=$tid#$att (async$([ "$wait" = 1 ] && echo '+wait' || true))"
}

cmd_wait() {
  # 默认 --any：先到先看。all 仅显式齐等（讨论期不推荐）。
  local mode=any tmo="${PAR_DISCUSS_WAIT_TIMEOUT_MS:-600000}" slot=""
  while [ $# -gt 0 ]; do
    case "$1" in
  ...
  _discuss_poll_token() {
    ...
    while [ "$elapsed" -lt "$budget_ms" ]; do
      _discuss_try_harvest_from_token "$name" "$p" && return 0
      sleep 0.3
      elapsed=$((elapsed + step))
    done
    _discuss_try_harvest_from_token "$name" "$p"
  }
  _discuss_wait_one() {
    local p=$1 name=$2 st pending busy harv tid att
    [ -n "$p" ] || { err "无 $name pane（先 open）"; return 1; }
    st=$(_discuss_agent_status "$p")
    if _discuss_round_takeable "$name" "$st" "$p"; then
      echo "parallel: discuss wait $name pane=$p already=token-ready"
      return 0
    fi
    read -r pending busy harv tid att < <(_discuss_round_read "$name")
    # 无本轮 pending：未 fire / 已 take —— 仅等 agent 离开 working
    if [ "$pending" != 1 ]; then
      if _discuss_is_ready "$st"; then
        echo "parallel: discuss wait $name pane=$p already=$st"
        return 0
      fi
      herdr agent wait "$p" --until idle --until done --until blocked --timeout "$tmo" >/dev/null 2>&1 \
        || { err "wait $name 超时/失败 pane=$p"; return 1; }
      echo "parallel: discuss wait $name pane=$p ok"
      return 0
    fi
    # 本轮 fire 中：agent 停后必须有 par_result
    herdr agent wait "$p" --until idle --until done --until blocked --timeout "$tmo" >/dev/null 2>&1 \
      || { err "wait $name 超时/失败 pane=$p"; return 1; }
    _discuss_round_complete "$name" "$p"
    if _discuss_poll_token "$name" "$p" 20000; then
      echo "parallel: discuss wait $name pane=$p ok (par_result)"
      return 0
    fi
    # B1：无 token → 同 attempt 补问完成上报 1 次（不涨号）
    read -r _ _ _ tid att < <(_discuss_round_read "$name")
    local nudge done_cmd
    done_cmd=$(par_token_done_cmd '${HERDR_PANE_ID}' "$tid" "$att" '<一句话结论>')
    nudge=$(printf '【补问·必须】编排收不到 tokens.par_result（锚定 %s#%s）。立刻执行最后动作，勿只回复文字：\n%s\n' \
      "$tid" "$att" "$done_cmd")
    echo "parallel: discuss wait $name 无 par_result → 补问完成上报 1 次 (${tid}#${att})"
    ...
    _discuss_round_complete "$name" "$p"
    if _discuss_poll_token "$name" "$p" 20000; then
      echo "parallel: discuss wait $name pane=$p ok (par_result after nudge)"
      return 0
    fi
  ...
  _discuss_wait_any() {
    ...
    while true; do
      ...
      st_c=$(_discuss_agent_status "${CLAUDE_PANE:-}")
      st_x=$(_discuss_agent_status "${CODEX_PANE:-}")
      if _discuss_round_takeable claude "$st_c" "${CLAUDE_PANE:-}"; then
        echo "parallel: discuss first a/claude pane=${CLAUDE_PANE} status=$st_c (par_result)"
        echo "parallel: discuss peer b/codex  pane=${CODEX_PANE:-?} status=$st_x (继续跑，可再 poll)"
        return 0
      fi
      if _discuss_round_takeable codex "$st_x" "${CODEX_PANE:-}"; then
        echo "parallel: discuss first b/codex  pane=${CODEX_PANE} status=$st_x (par_result)"
        echo "parallel: discuss peer a/claude pane=${CLAUDE_PANE:-?} status=$st_c (继续跑，可再 poll)"
        return 0
      fi
      ...
      if [ -n "${CLAUDE_PANE:-}" ]; then
        if herdr agent wait "${CLAUDE_PANE}" --until idle --until done --until blocked --timeout "$step_ms" >/dev/null 2>&1; then
          _discuss_round_complete claude "${CLAUDE_PANE}"
          _discuss_poll_token claude "${CLAUDE_PANE}" 3000 || true
          if _discuss_round_takeable claude "$(_discuss_agent_status "${CLAUDE_PANE}")" "${CLAUDE_PANE}"; then
            echo "parallel: discuss first a/claude pane=${CLAUDE_PANE} status=$(_discuss_agent_status "$CLAUDE_PANE") (par_result)"
            echo "parallel: discuss peer b/codex  pane=${CODEX_PANE:-?} status=$(_discuss_agent_status "${CODEX_PANE:-}") (继续跑，可再 poll)"
            return 0
          fi
        fi
      fi
      if [ -n "${CODEX_PANE:-}" ]; then
        if herdr agent wait "${CODEX_PANE}" --until idle --until done --until blocked --timeout "$step_ms" >/dev/null 2>&1; then
          _discuss_round_complete codex "${CODEX_PANE}"
          _discuss_poll_token codex "${CODEX_PANE}" 3000 || true
          if _discuss_round_takeable codex "$(_discuss_agent_status "${CODEX_PANE}")" "${CODEX_PANE}"; then
            echo "parallel: discuss first b/codex  pane=${CODEX_PANE} status=$(_discuss_agent_status "$CODEX_PANE") (par_result)"
            echo "parallel: discuss peer a/claude pane=${CLAUDE_PANE:-?} status=$(_discuss_agent_status "${CLAUDE_PANE:-}") (继续跑，可再 poll)"
            return 0
          fi
        fi
      fi
  ...
  case "$mode" in
    claude) _discuss_wait_one "$CLAUDE_PANE" claude || exit 2 ;;
    codex)  _discuss_wait_one "$CODEX_PANE"  codex  || exit 2 ;;
    any)    _discuss_wait_any || exit 2 ;;
    all)
      fail=0
      _discuss_wait_one "$CLAUDE_PANE" claude || fail=1
      _discuss_wait_one "$CODEX_PANE"  codex  || fail=1
      [ "$fail" = 0 ] || exit 2
      ;;
  esac
}
...
cmd_take() {
  ...
  _discuss_round_takeable claude "$st_c" "${CLAUDE_PANE:-}" && ta=true
  _discuss_round_takeable codex "$st_x" "${CODEX_PANE:-}" && tb=true
  ...
  case "$want" in
    first)
      if [ "$ta" = true ]; then _discuss_sel_push a/claude "$CLAUDE_PANE" "$st_c" claude
      elif [ "$tb" = true ]; then _discuss_sel_push b/codex "$CODEX_PANE" "$st_x" codex
      fi ;;
    all)
      [ "$ta" = true ] && _discuss_sel_push a/claude "$CLAUDE_PANE" "$st_c" claude
      [ "$tb" = true ] && _discuss_sel_push b/codex "$CODEX_PANE" "$st_x" codex
      ;;
    slot:claude) [ "$ta" = true ] && _discuss_sel_push a/claude "$CLAUDE_PANE" "$st_c" claude ;;
    slot:codex)  [ "$tb" = true ] && _discuss_sel_push b/codex  "$CODEX_PANE"  "$st_x"  codex  ;;
  esac
  ...
  if [ "$json" = 1 ]; then
    ...
    for i in "${!sel_name[@]}"; do
      tok=$(_par_raw_token "${sel_pane[$i]}")
      echo "parallel: discuss take ${sel_name[$i]} pane=${sel_pane[$i]} status=${sel_st[$i]}"
    ...
    if [ "$n" -ge 1 ]; then
      echo "parallel: discuss take done count=$n"
    else
      echo "parallel: discuss take none (无本轮 par_result) — 可 fire 或稍后再 take"
    fi
  fi
  # L3：轮次归档（token 结论落盘，再清 token）
  local i
  for i in "${!sel_role[@]}"; do
    _discuss_archive_take "${sel_role[$i]}" "${sel_pane[$i]}"
    _discuss_round_took "${sel_role[$i]}" "${sel_pane[$i]}"
  done
  [ "$n" -ge 1 ] && return 0
  return 3
}

# 归档 .parallel/discuss-*/archive/<attempt>-<ts>.md（PAR_DISCUSS_ARCHIVE=0 关闭）
_discuss_archive_take() {
  local role=$1 pane=$2
  [ "${PAR_DISCUSS_ARCHIVE:-1}" = "0" ] && return 0
  local d pending busy harv tid att tok f ts
  d=$(_discuss_dir "$role") || return 0
  read -r pending busy harv tid att < <(_discuss_round_read "$role")
  ...
  echo "parallel: discuss archive $f"
}
...
cmd_collect() {
  local wait_mode=all take_args=(--all) do_read=1 tmo="${PAR_DISCUSS_WAIT_TIMEOUT_MS:-600000}"
  while [ $# -gt 0 ]; do
    case "$1" in
  ...
  echo "parallel: discuss collect（wait→take 编排糖；fire 仍异步两令）"
  case "$wait_mode" in
...
cmd_close() {
  ...
  rm -f "$DISCUSS_CLAUDE_DIR/pane" "$DISCUSS_CODEX_DIR/pane" \
    "$DISCUSS_CLAUDE_DIR/round" "$DISCUSS_CODEX_DIR/round" 2>/dev/null || true
  for d in "$DISCUSS_CLAUDE_DIR" "$DISCUSS_CODEX_DIR"; do
    [ -d "$d" ] && echo discarded > "$d/state" 2>/dev/null || true
  done
  if [ -f "$DISCUSS_ROOT/session" ]; then
    echo "closed_at=$(date -Iseconds 2>/dev/null || date)" >> "$DISCUSS_ROOT/session"
  fi
  echo "parallel: discuss closed"
}

# archive：归档清单/汇总（P-3）。扫 .parallel/discuss-*/archive/*.md，逐条解 role/attempt/时间/结论
# rc0 有条目；rc3 无归档
cmd_archive() {
  local json=0
  while [ $# -gt 0 ]; do
  ...
  local root="${DISCUSS_ROOT%/discuss}" f role att at tok n=0
  local items='[]'
  ...
  done < <(find "$root" -maxdepth 3 -path '*discuss-*/archive/*.md' 2>/dev/null | sort)
  if [ "$json" = 1 ]; then
    jq -n --argjson items "$items" --argjson n "$n" '{count:$n, archive:$items}'
  else
    echo "parallel: discuss archive count=$n"
  fi
  [ "$n" -ge 1 ] && return 0
  return 3
}
...
case "$sub" in
  ...
  *)
    case "$sub" in
      --*) cmd_open "$sub" "$@" ;;
      *) err "usage: par-discuss.sh open|fire|take|collect|status|poll|wait|close …"; exit 2 ;;
    esac
    ;;
esac
